...
import pandas as pd
from salt_func import calc_rev_diff, sim_salt
...
for single_amount in tqdm(range(0, 10001, 100)[34:]):
    model = sim_salt(single_amount, sub, cg, with_penalty=True)
    ...
    df.to_pickle('salt_results_with_MP/'+str(single_amount)+'.pkl')
    ...
    df_item.to_pickle('salt_results_with_MP/'+str(single_amount)+'_itemizers.pkl')
